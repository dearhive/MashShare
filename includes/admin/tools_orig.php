	...
	<div class="postbox">
		...
		<div class="inside">
			...
			<form method="post" action="<?php echo admin_url( 'admin.php?page=mashsb-tools&tab=import_export' ); ?>">
				<p><input type="hidden" name="mashsb_action" value="export_settings" /></p>
				<p>
...
echo "tester";
	...
	header( 'Content-Disposition: attachment; filename=' . apply_filters( 'edd_settings_export_filename', 'mashsb-settings-export-' . date( 'm-d-Y' ) ) . '.json' );
	header( "Expires: 0" );

	echo json_encode( $settings );
	exit;
}
add_action( 'edd_export_settings', 'mashsb_tools_import_export_process_export' );
...
add_action( 'edd_import_settings', 'mashsb_tools_import_export_process_import' );
...
?>
	<form action="<?php echo esc_url( admin_url( 'edit.php?post_type=download&page=edd-tools&tab=system_info' ) ); ?>" method="post" dir="ltr">
		<textarea readonly="readonly" onclick="this.focus(); this.select()" id="system-info-textarea" name="edd-sysinfo" title="To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac)."><?php echo mashsb_tools_sysinfo_get(); ?></textarea>
		<p class="submit">
...
add_action( 'edd_download_sysinfo', 'mashsb_tools_sysinfo_download' );
