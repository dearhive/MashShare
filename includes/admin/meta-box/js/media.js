...
function twitterRemaining() {
	...
	if(smTwitter.length > 0 && remaining >= 0) { jQuery('#mashsb_meta .mashsb-custom-tweet .mashsb_counter').removeClass('mashsb_exceeded');	
	} else if (smTwitter.length > 0 && remaining < 0 ) { jQuery('#mashsb_meta .mashsb-custom-tweet .mashsb_counter').addClass('mashsb_exceeded');
	} else { jQuery('#mashsb_meta .mashsb-custom-tweet .mashsb_counter').removeClass('mashsb_exceeded'); }
...
jQuery( function ( $ ) {
	if(jQuery('#mashsb_meta.postbox').length) {
		...
		smTitleRemaining();
		jQuery('#mashsb_meta .mashsb-og-title textarea').on('input', function() { smTitleRemaining(); });
		
		smDescriptionRemaining();
		jQuery('#mashsb_meta .mashsb-og-desc textarea').on('input', function() { smDescriptionRemaining(); });
		
		twitterRemaining();
		jQuery('#mashsb_custom_tweet').on('input', function() { twitterRemaining(); });
